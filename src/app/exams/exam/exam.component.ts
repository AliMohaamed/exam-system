...
import {
  StudentExam,
  Question,
} from '../../core/models/student-exam.interface';
...
export class ExamComponent implements OnInit, OnDestroy {
      ...
      this.examService.startExam(this.examDetails._id).subscribe({
        next: (response) => {
          ...
          if (response.success) {
            ...
            this.alertService.error(response.message || 'Failed to start exam');
          }
        },
        error: (error) => {
          ...
          this.alertService.error(
            error.error?.message || 'Failed to start exam. Please try again.'
          );
        },
      });
    }
  ...
  private startTimer(duration: number) {
    ...
    this.interval = window.setInterval(() => {
      if (this.timeLeft > 0) {
        ...
        this.alertService.warning('Time is up! Your exam is being submitted.');
  ...
  submitExam() {
    ...
    this.examService
      .submitExamAnswers(this.attemptId, { answers: answersArray })
      .subscribe({
        next: (response) => {
  ...
  async onPopState(event: PopStateEvent) {
    if (this.isExamStarted && !this.showResult) {
      if (
        await this.alertService.confirm('auto submit. No retake. Continue?')
      ) {
        this.submitExam();
      } else {
  ...
  async canDeactivate(): Promise<boolean> {
    if (this.isExamStarted && !this.showResult) {
      const result = await this.alertService.confirm(
        'auto submit. No retake. Continue?'
      );
      if (result) {
        this.submitExam();
        return true;
